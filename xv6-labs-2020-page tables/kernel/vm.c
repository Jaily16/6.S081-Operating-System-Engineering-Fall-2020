 ...
 */
// create a kernel page
pagetable_t
kvmmake()
{
  // request memory for kernel table
...
// Free n users memory pages start va,
// then free page-table pages.
void
uvmfree2(pagetable_t ptable, uint64 va, uint n)
{
  if(n > 0)
    uvmunmap(ptable, va, n, 1);
  freewalk(ptable);
}
...
// 遍历打印页表的信息
static void traversal_print(pagetable_t ptable, int level)
{
  for(int i = 0; i < 512; i++)
  {
    pte_t pte = ptable[i];
    if(pte & PTE_V)
    {
      ...
      {
        printf("..%d: pte %p pa %p\n", i, pte, child);
        traversal_print((pagetable_t)child, level + 1);
      }
      else if(level == 1)
      {
        printf(".. ..%d: pte %p pa %p\n", i, pte, child);
...
// 打印PTE信息
void vmprint(pagetable_t ptable)
{
  printf("page table %p\n", ptable);
  traversal_print(ptable, 0);
}
