...
void
proc_free_kernel_pagetable(uint64 kstack, pagetable_t ptable, uint64 sz)
{
  uvmunmap(ptable, UART0, 1, 0);
  uvmunmap(ptable, VIRTIO0, 1, 0);
  // uvmunmap(ptable, CLINT, 0x10000/PGSIZE, 0);
  uvmunmap(ptable, PLIC, 0x400000/PGSIZE, 0);
  uvmunmap(ptable, KERNBASE, ((uint64)etext - KERNBASE)/PGSIZE, 0);
  uvmunmap(ptable, (uint64)etext, (PHYSTOP - (uint64)etext)/PGSIZE, 0);
  uvmunmap(ptable, TRAMPOLINE, 1, 0);
  uvmunmap(ptable, 0, PGROUNDUP(sz)/PGSIZE, 0);

  uvmfree2(ptable, kstack, 1);
}
